 ...
 *
 * Returns every risky node with:
 *   • Its CVEs
 *   • A dynamic explanation derived from RBAC topology + CVE data
 *   • A short summary reason
 *
 * Query params:
 *   minRisk  — minimum riskScore to include (default 3, range 0–10)
 *
 * Explanations are generated by risk-explainer.ts — NOT hardcoded strings.
 * Every sentence is derived from the node's actual relationships and CVE data.
 */
router.get('/', async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    // Single query: fetch all risky nodes with their in/out edge context.
    // OPTIONAL MATCH produces null rows when no edges exist — filter them out
    // in the WITH clause using CASE WHEN.
    ...
    // Build enriched vulnerability response
